// Global keydown event to prevent Enter key abuse
document.addEventListener('keydown', function(e) {
    // Prevent multiple Enter key presses on confirmation dialogs
    if (e.key === 'Enter') {
        const confirmationDialogs = document.querySelectorAll('.confirmation-dialog');
        if (confirmationDialogs.length > 0) {
            // Process only the first Enter press on each dialog
            const firstDialog = confirmationDialogs[0];
            const confirmBtn = firstDialog.querySelector('.confirm-action');
            
            if (confirmBtn && !confirmBtn.disabled) {
                e.preventDefault();
                // Simulate a click instead of letting Enter continue
                confirmBtn.click();
                confirmBtn.disabled = true;
            } else if (confirmBtn && confirmBtn.disabled) {
                // Prevent further Enter presses if button is disabled
                e.preventDefault();
            }
        }
    }
...
// Global function to prevent multiple form submissions
function preventMultipleSubmissions(formElement) {
    formElement.addEventListener('submit', function(e) {
        ...
        // Reset the flag after form processing completes
        setTimeout(function() {
...
// Function to prevent multiple dialog popups
function preventMultipleDialogs() {
    // Remove any existing confirmation dialogs before creating a new one
